...
class PCENetDataset(object):
    def __init__(self, features, labels, knn_graph, dists, sims, k, scores):
        ...
        density_diff_map = self.density[self.knn_graph[:]] > np.expand_dims(self.density[self.knn_graph[:]][:, 0],
                                                                            axis=1)
        ...
        E_s[:, 0] = 0
        # our test
        temp_sim = self.sims[:, 1:]
        test_sorted_sim = np.sort(temp_sim, axis=1)
        sim_threshold = np.mean(test_sorted_sim[:, 76:80])
        sim_threshold = np.mean(np.average(test_sorted_sim[:, 76:80], axis=1,weights=[0.1, 0.2, 0.7]))
        # sim_threshold = 0.975
